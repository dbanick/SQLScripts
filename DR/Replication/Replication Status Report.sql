...
CREATE TABLE [dbo].[#ReplStats] (
    ...
    ,[UndelivCmdsInDistDB] [bigint] NULL
    ,[DelivCmdsInDistDB] [bigint] NULL
    ,[CurrentSessionDeliveryRate] [float] NOT NULL
    ,[CurrentSessionDeliveryLatency] [int] NOT NULL
    ,[TotalTransactionsDeliveredInCurrentSession] [bigint] NOT NULL
    ,[TotalCommandsDeliveredInCurrentSession] [bigint] NOT NULL
    ,[AverageCommandsDeliveredInCurrentSession] [int] NOT NULL
    ,[DeliveryRate] [float] NOT NULL
    ,[DeliveryLatency] [int] NOT NULL
    ,[TotalCommandsDeliveredSinceSubscriptionSetup] [bigint] NOT NULL
...
FROM [DATA_distribution]..[MSdistribution_status] ds WITH(NOLOCK)
INNER JOIN [DATA_distribution]..[MSdistribution_agents] da WITH(NOLOCK) ON da.[id] = ds.[agent_id]
INNER JOIN [DATA_distribution]..[MSArticles] ma ON ma.publisher_id = da.publisher_id
    AND ma.[article_id] = ds.[article_id]
INNER JOIN [DATA_distribution]..[MSreplication_monitordata] md  WITH(NOLOCK) ON [md].[job_id] = da.[job_id]
INNER JOIN [DATA_distribution]..[MSdistribution_history] dh  WITH(NOLOCK) ON [dh].[agent_id] = md.[agent_id]
    AND md.[agent_type] = 3
...
WHERE da.[subscriber_db] <> 'virtual'
    ...
    AND dh.[start_time] = (
        ...
        FROM [DATA_distribution]..[MSdistribution_history] a
        INNER JOIN [DATA_distribution]..[MSdistribution_agents] b ON a.[agent_id] = b.[id]
