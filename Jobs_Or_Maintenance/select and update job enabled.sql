...
select j.name as job, j.enabled as 'enabled' from msdb.dbo.sysjobs j
--INNER JOIN MSDB.dbo.syscategories c ON j.category_id = c.category_id
WHERE j.[Name] like '%LSCopy%' or j.[Name] like '%LSRestore%' and j.enabled = 1
order by j.name
...
WHERE j.[Name] like '%LSCopy%' or j.[Name] like '%LSRestore%'
and j.enabled = 1
...
BEGIN
SELECT 'EXEC msdb.dbo.sp_update_job @job_id=@id, @enabled=0'
EXEC msdb.dbo.sp_update_job @job_id=@id, @enabled=0
