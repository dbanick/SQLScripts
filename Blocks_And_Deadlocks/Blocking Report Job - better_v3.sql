--Changes to make
--update Email profile/account and Recipients --line 161 & 162
...
DECLARE @kill bit = 0 -- 0 will report only, 1 will kill the blocker
...
FROM [DBA].[dbo].[WhoisactiveLogging] a
JOIN [DBA].[dbo].[WhoisactiveLogging] b ON a.[blocking_session_id] = b.[session_id]
WHERE a.[blocking_session_id] IS NOT NULL
	AND CONVERT(INT, SUBSTRING(a.wait_info, 2, CHARINDEX('ms)', a.wait_info) - 2)) > 120000
	AND a.[collection_time] >= DATEADD(minute, - 15, GETDATE())
	AND a.database_name in ('MCAddons', 'MCBuy', 'MCMain', 'MCPersonal', 'MCWork')
	;
...
      */
	...
	IF @kill = 0
		SELECT @subj = 'Blocking Alert'
	ELSE
		SELECT @subj = 'Lead Blocking Killed'
	-- Send alert via database mail 
	EXEC msdb.dbo.sp_send_dbmail @profile_name = 'DB_Admin_Profile'
		,@recipients = 'sbeatrice@composecure.com; nick.patti@datastrike.com; lucja.dolega@datastrike.com'
		,-- 
		@subject = @subj
		,@body = @tableHTML
		,        @body_format = 'HTML'
	...
	--kill blocking if option is set
	IF @kill = 1 
	BEGIN
		DECLARE @killcmd varchar(200);
		DECLARE @BlockerSessionID INT;

		DECLARE BlockerCursor CURSOR LOCAL FAST_FORWARD FOR
			SELECT distinct blocker_session_id
			FROM ##LatestBlocking;

		OPEN BlockerCursor;

		FETCH NEXT FROM BlockerCursor INTO @BlockerSessionID;

		WHILE @@FETCH_STATUS = 0
		BEGIN
			SET @killcmd = 'KILL ' + cast(@BlockerSessionID as varchar(20))
			exec @killcmd
			FETCH NEXT FROM BlockerCursor INTO @BlockerSessionID;
		END

		CLOSE BlockerCursor;
		DEALLOCATE BlockerCursor;
	END
