...
If @version = 8
	Begin
		...
		name as 'Job Name',
		active_start_time,
		case when ScheduleDscr is null then '*Job is not scheduled' else ScheduleDscr end as 'Schedule',
...
If @version > 8
	Begin
		set @cmd = "
		select name as 'Job Name', active_start_time, 
		case when ScheduleDscr is null then '*Job is not scheduled' else ScheduleDscr end, 
		case when enabled = 1 then 'Yes' else 'No' end as 'Enabled',
		(select name from dbo.syscategories where category_id = catID) As 'Category',
		case when [Status] = 0 then 'Failed'
		when [Status] = 1 then 'Succeeded'
