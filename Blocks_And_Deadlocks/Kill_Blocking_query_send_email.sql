...
--By default this will look for anything older than 2 hours
SET @minutes = 120
...
--Kill the query
IF EXISTS (SELECT 1 FROM #BlockProcs WHERE   [DBCC INPUTBUFFER] LIKE '%DELETE FROM %'
						and BlkBy = 0 and LASTBATCH < DATEADD(MINUTE, @minutes, GETDATE()) and [LOGIN] = 'LoginName'
			)
BEGIN
	...
	SET @QKILLsp= (SELECT DISTINCT '  KILL '+ CONVERT(VARCHAR,SPID)
							FROM #BlockProcs I
							WHERE   [DBCC INPUTBUFFER] LIKE '%DELETE FROM %'
							and BlkBy = 0 and LASTBATCH < DATEADD(MINUTE, -15, GETDATE())
							and [LOGIN] = 'LoginName'
							for XML path('')
							)
	...
	IF @Debug = 1
		...
		IF @EmailReport = 1
						...
						<br />
						<br />
						The following command has been ran to clear the blocking:
						<br />
						 ' + cast(@QKILLsp as varchar(10)) + '
						 <br />
						 It was causing blocking and was open for ' + cast(@minutes as varchar(10))+ ' minutes.
						</body></html>';
						...
						Set @Body = Replace(@Body, '<TRRow>0</TRRow>', '')
							...
							EXEC msdb.dbo.sp_send_dbmail @recipients='npatti@rdx.com;Mspelda@girlscouts.org', -- change mail address accordingly
								@subject = 'Blocking Session Detected', 
								@profile_name = 'email', -- Change profile name accordingly
								@body = @Body,
								@body_format = 'HTML' ;
